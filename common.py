...
# Need to figure out how to increment t outside/inside of play_n_episodes

# t = 0
# File to output actions to, change name for each game.
# f = open("Chopper Command/chopper_command_act.txt", "w")
...
def play_one_episode(env, func, render=False):
    ...
    while True:
        ...
        # Spyros Code to save images - I added t
        stacker = np.empty((84, 0, 3), dtype="uint8")
        for it in range(4):
            # im = np.empty((84, 0, 3), dtype="uint8")
            im = Image.fromarray(ob[:, :, it * 3:3 * (it + 1)])
            q = np.asarray(im)
            stacker = np.hstack((stacker, q))
        im = Image.fromarray(stacker)

        # if t < 3001:
        #     im.save("Chopper Command/choppercommand" + str(t) + ".png")
        #     # Output action to text
        #     f.write(str(act) + "\n")
        #     t += 1
        #     print ("T: ", t)
